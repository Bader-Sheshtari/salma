    ...
    return (
      <form action={formAction} className="flex flex-1 flex-wrap items-center gap-2">
        ...
        <input
          ...
          className="w-20 rounded-lg border border-gray/40 bg-white px-2 py-2 text-sm outline-none focus:border-teal"
        />
        <button
          disabled={pending}
  ...
  return (
    <form action={formAction} className="flex flex-col gap-3 rounded-2xl border border-line bg-white p-4">
      ...
      <div className="grid gap-3 sm:grid-cols-[1fr_auto_auto]">
        ...
        <input name="sort_order" type="number" placeholder="الترتيب" className="w-28 rounded-lg border border-gray/40 bg-white px-3 py-2 text-sm outline-none focus:border-teal" />
        <button
          disabled={pending}
