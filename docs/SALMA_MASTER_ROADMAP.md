...
1. **Minimum viable monitoring** — **PARTIALLY DONE (2026-08-21).** radar-rank now has run-level observability (see the incident fix below): a `running` row is written **before** any work and finalized to success/partial/failed, with `backlog_before`/`backlog_after`/`skipped`; a compact `radar_rank_health` view exposes last-success, last-run-status, **stale `running` runs** (crash detection), and current backlog. **Still open:** the same treatment for ESL (`radar-editorial-select` has no run table or top-level try/catch — a crash there is still invisible) and the other collectors; one simple alert channel (email/webhook) for cron/edge failures; a stale-pipeline ("no candidates today") check; a tiny admin health strip reading `radar_rank_health` (+ future ESL health). No external alerting yet — the health view makes the problem *visible on query*, not *pushed*.
